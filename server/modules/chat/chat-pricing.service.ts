...
export class ChatPricingService {
  ...
  public detectMarketPriceQuestion(content: string): boolean {
    const patterns = [
      ...
      /(住家|白班|育儿|护工|菲式|钟点|保姆|阿姨).{0,8}(一般|大概|差不多)(多少|几|价|价位)/,
      // "住家保姆多少钱 / 育儿嫂价格" —— 必须含钱/块/元/价/位/价格/费用等钱相关词
      /(住家|白班|育儿|护工|菲式|钟点|保姆|阿姨).{0,5}(多少钱|多少块|多少元|什么价|什么价位|价位|价格是多少|价格怎么样|费用是多少|费用大概)/,
      // "保姆怎么收费 / 住家保姆怎么收费" —— 服务类型+怎么收费=问价格
      /(住家|白班|育儿|护工|菲式|钟点|保姆|阿姨).{0,5}怎么收费/,
      // "X 一个月多少钱 / X 月薪多少"
   ...
   *   2) serviceType 是其他 5 类（白班/育儿/护工/菲式/钟点工）→ 说差异大要看具体需求
   ...
   */
   public async buildMarketPriceCanonical(
    ...
    // 分支 2：客户已确认是其他服务类型（白班保姆/育儿保姆/护工/菲式/钟点工）→ 查表给区间
    // 2026-08-16 林琳 20:29 拍板：已采集城市时**只**给该城市档位（之前 v4 bug：只 sort 不 filter，输出所有 tier 跟未采集城市一样）
    if (serviceType) {
      const otherTypes = ['白班保姆', '育儿保姆', '育儿嫂', '护工', '菲式', '钟点工'];
